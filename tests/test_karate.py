...
class KarateTest(unittest.TestCase):
    ...
    def test_GraphFactorization(self):
        model = GraphFactorization(
            d=2, max_iter=50000, eta=1 * 10**-4, regu=1.0, data_set="karate"
        )
        target = np.loadtxt(
            os.path.join(self.source_dir, "karate_res/GraphFactorization.txt")
        )
    ...
    def test_LaplacianEigenmaps(self):
        ...
        target = np.loadtxt(
            os.path.join(self.source_dir, "karate_res/LaplacianEigenmaps.txt")
        )
        self.internal_model_test(model, target, mae_close=True)
    ...
    def test_LocallyLinearEmbedding(self):
        ...
        target = np.loadtxt(
            os.path.join(self.source_dir, "karate_res/LocallyLinearEmbedding.txt")
        )
        self.internal_model_test(model, target, mae_close=True)

    def test_node2vec(self):
        model = node2vec(
            d=2, max_iter=1, walk_len=80, num_walks=10, con_size=10, ret_p=1, inout_p=1
        )
    ...
    def internal_model_test(
        self, model, target, verbose: bool = False, mae_close: bool = False
    ):
        ...
        if not mae_close:
            ...
            self.assertTrue(abs(np.mean(target - model.get_embedding())) < 0.3)
